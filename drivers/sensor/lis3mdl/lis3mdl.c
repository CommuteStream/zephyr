...
int lis3mdl_sample_fetch_drv(struct lis3mdl_data *drv_data, enum sensor_channel chan) {
  ...
  __ASSERT_NO_MSG(chan == SENSOR_CHAN_TEMP || chan == SENSOR_CHAN_MAGN_XYZ);
...
{
	struct lis3mdl_data *drv_data = dev->driver_data;
	return lis3mdl_sample_fetch_drv(drv_data, chan);
}
...
DEVICE_INIT(lis3mdl, CONFIG_LIS3MDL_DEV_NAME, lis3mdl_init, &lis3mdl_driver,
